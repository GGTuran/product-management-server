...
const variantZodSchema = z.object({
    type: z.string({errorMap:()=>({message:'Variant type should be a string'})}),
    value: z.string({errorMap:()=>({message:'Variant value should be a string'})}),
  });
  

  const inventoryZodSchema = z.object({
    quantity: z.number({errorMap:()=>({message:'quantity should be a number'})}).nonnegative(),
    inStock: z.boolean({errorMap:()=>({message:'The number of available stocks should be a number'})}),
  });
  ...
  const productZodSchema = z.object({
    ...
    tags: z.array(z.string({errorMap:()=>({message:'Tags are invalid'})})),
